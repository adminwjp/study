<template>
	<view class="center">
        ...
        <picture-tabbar></picture-tabbar>
	</view>
</template>

<script>
    import pictureTabbar from '../picture-tabbar.vue'
	export default {
        components:{pictureTabbar},
		data() {
			return {
				login: false,
		...
		methods: {
			goLogin() {
				if (!this.login) {
					uni.navigateTo({
						url: '/pages/socialconat/picture/login/login'
					});
				}
			},
			goAbout() {
				uni.navigateTo({
					url: '/pages/socialconat/picture/about/about'
				});
			}
		}
	}
