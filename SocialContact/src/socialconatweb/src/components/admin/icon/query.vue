  ...
  <el-form :inline="true" :model="formQuery" ref="formQuery" class="demo-form-inline query">
    <el-form-item label="图标分类" prop="id">
      <el-select
        ...
        filterable
        placeholder="请选择"
        @change="handleCategoryQueryChangeEvent"
        @visible-change="handleCategoryQueryVisableChangeEvent"
      >
        <el-option v-for="item in options" :key="item.label" :label="item.label" :value="item.id">
          <span style="float: left">{{ item.label }}</span>
          <span style="float: right; color: #8492a6; font-size: 13px">
            <i v-bind:class="item.value"></i>
          </span>
        </el-option>
      </el-select>
    </el-form-item>
    ...
    <!-- <defaultQuery :formQuery="formQuery" /> -->
  <el-form-item label="创建时间范围" prop="create_date">
        <el-date-picker
          v-model="formQuery.create_date"
          type="datetimerange"
          value-format="yyyy-MM-dd HH:mm:ss"
          :picker-options="createDatePickerOptions"
          range-separator="至"
