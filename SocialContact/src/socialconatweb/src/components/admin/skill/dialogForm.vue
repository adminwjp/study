            ...
            <el-form :model="formSubmit" status-icon :rules="formRoleValidator" :disabled="disabled" ref="formSubmit" class="demo-ruleForm" :label-position="labelPosition" label-width="80px">
                ...
                <template v-if="test">
                    <el-form-item label="分类" prop="category.id">
                        <el-select v-model="formSubmit.category.id" filterable placeholder="请选择" @change="handleCategoryChangeEvent"
                        @visible-change="handleCategoryVisableChangeEvent">
...
methods.handleCategoryChangeEvent = function(val) {
  this.$parent.handleCategoryChangeEvent(val);
};
methods.handleCategoryVisableChangeEvent = function(val) {
  this.$parent.handleCategoryVisableChangeEvent(val);
};
